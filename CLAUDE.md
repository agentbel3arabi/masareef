...
| [superpowers/specs/](./docs/superpowers/specs/) | High-level design specs and sprint plans per phase | Starting any phase — check for a matching spec |
| [superpowers/plans/](./docs/superpowers/plans/) | Detailed implementation plans per phase | Executing a phase — follow task-by-task |
...
| Phase N implementation         | `CLAUDE.md` + matching `superpowers/specs/` + matching `superpowers/plans/`                       | `02-data-models.md`, relevant feature specs, `guides/09-design-tokens.md` if frontend |
...
10. **Session handoff notes are mandatory.** At the **start** of any implementation unit, read the most recent handoff note in `docs/superpowers/handoff/` before writing any code. At the **end**, create a new handoff note at `docs/superpowers/handoff/phase-N-unit-X.md` using the template at `docs/handoff-template.md`, commit it to main, and push.

11. **Async-first backend.** Build with `async def` everywhere. Use asynchronous SQLAlchemy for all DB interactions. Use **`fastapi.BackgroundTasks`** for fire-and-forget logic. Use **APScheduler** for recurring cron jobs.

12. **Pydantic V2 only.** Call `model.model_dump()` exclusively; `model.dict()` is **forbidden**. Use `model_config = {"from_attributes": True}` for ORM mapping.

13. **uv for dependency management** (`pyproject.toml` + `uv.lock`). Do not use Poetry, Conda, or `pip install` directly.

14. **Backlog is mandatory.** At the end of every implementation unit, extract all deferred items, bugs, tech debt, and new ideas into `BACKLOG.md` with a `BL-NNN` ID. At the start of every phase plan, pull all items tagged for that phase and either include them in the plan or explicitly re-defer them. At phase completion, archive resolved items to `docs/backlog-archive.md`. See `BACKLOG.md` header for format and taxonomy.
